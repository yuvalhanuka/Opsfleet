...
class BigQueryRunner:
    ...
    def execute_query(self, sql_query: str) -> pd.DataFrame:
        ...
        try:
            ...
            sql_query = self.strip_sql_fence(text=sql_query)
    ...
    def strip_sql_fence(text: str) -> str:
        """
        If `text` starts with '```sql' and ends with '```', remove those fences.
        Preserves the inner content, trimming a single leading newline after '```sql'
        ...
        if s.startswith("```sql") and s.endswith("```"):
            ...
            # Remove the closing '```'
            inner = inner[:-3]
            # Optionally trim a trailing newline introduced before the fence
            return inner.rstrip("\r\n")
        return text
